...
namespace BA_MobileGPS.Core.ViewModels
{
    public class HomeViewModel : ViewModelBase
    {
        ...
        private void GenMenu()
        {
            ...
            var menus = MenuReponse.Where(x => permisisons.Contains(x.PermissionViewID)
                                               && x.MenuItemParentID != 0
                                               && x.MenuItemParentID != x.PK_MenuItemID).ToList();
            StaticSettings.ListMenuOriginGroup = mapper.Map<List<HomeMenuItem>>(menus);
            GenerateListFeatures(menus);
            GenerateFavourites(menus);
        }
        ...
        private void GenerateFavourites(List<HomeMenuItemViewModel> input)
        {
            HasFavorite = false;
            // Lấy danh sách menu ưa thích theo danh sách chuối PK_MenuItemID
        ...
        public ICommand NavigateToFavoriteCommand => new Command(() =>
        {
            SafeExecute(async () =>
            {
                _ = await NavigationService.NavigateAsync("FavoritesConfigurationsPage", useModalNavigation: false);
            });
        });
